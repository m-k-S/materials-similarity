...
def get_boltztrap_data():
    df = load_dataset("boltztrap_mp", data_home="./data", pbar=False)
    df = df.drop(columns=['mpid', 'formula'])
    return df

def get_matbench_jdft2d_data():
    df = load_dataset("matbench_jdft2d", data_home="./data", pbar=False)
    return df

def get_matbench_mp_e_form_data():
    df = load_dataset("matbench_mp_e_form", data_home="./data", pbar=False)
    return df
